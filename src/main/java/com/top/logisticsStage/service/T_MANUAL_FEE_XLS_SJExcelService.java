...
@Service
@Transactional
public class T_MANUAL_FEE_XLS_SJExcelService {

    private final Logger log = LoggerFactory.getLogger(T_MANUAL_FEE_XLS_SJExcelService.class);

    private final String EXCELL_NAME = "classpath:templates/T_MANUAL_FEE_XLS_SJ.xlsx";
    private final T_MANUAL_FEE_XLS_SJRepository t_MANUAL_FEE_XLS_SJRepository;

    public T_MANUAL_FEE_XLS_SJExcelService(T_MANUAL_FEE_XLS_SJRepository t_MANUAL_FEE_XLS_SJRepository) {
    ...
    public void excelImport(Workbook workbook){
        ...
        try {
            ...
            int errorCount = 0;
            List<T_MANUAL_FEE_XLS_SJ> list = new ArrayList<>();
            for (int i = 1; i < totalRows; i++) {
                ...
                T_MANUAL_FEE_XLS_SJ t = mapToBean(entity);
                list.add(t);
                //错误信息记录
    ...
    public void tmpToEntity(List<T_MANUAL_FEE_XLS_SJ> list) {
        DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        try {
            t_MANUAL_FEE_XLS_SJRepository.saveAll(list);
        } catch (Exception e) {
    ...
    public byte[] downloadFile(Long excelId){
        ...
        try {
            ...
            List<T_MANUAL_FEE_XLS_SJ> list = t_MANUAL_FEE_XLS_SJRepository.findAll();
            // 创建绘图对象
    ...
    public T_MANUAL_FEE_XLS_SJ mapToBean(Map<String, Object> map) throws Exception {
        T_MANUAL_FEE_XLS_SJ bean = new T_MANUAL_FEE_XLS_SJ();
        ...
        bean.setPeriod(map.get("period")==null?null:map.get("period").toString());
        bean.setWithholding(map.get("withholding")==null?null:map.get("withholding").toString());
        ...
        bean.setActual(map.get("actual")==null?null:new BigDecimal(map.get("actual").toString()).setScale(2, RoundingMode.HALF_UP));
        return bean;
    }
